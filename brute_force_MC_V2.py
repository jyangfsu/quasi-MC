...
@nb.njit(nogil=True)
def normal_MC(n):
    param_values = param_values_base[:n, :]
    Y = cmpt_Dscs(param_values)
    N = Y.shape[1]
    Var_t_d = np.var(Y)
...
with ThreadPoolExecutor as executor:
    ...
    master_list = list(executor.map(normal_MC, np.arange(499, N)))
    
    
    
'''
    #%% Save the results
# ret = np.array(master_list)

ret = np.load('ST_C_normal_500.npy')
# np.save('ST_C_normal_250V2.npy', ret)

plt.figure(figsize=(6, 3))
plt.plot(np.arange(2, 500)**3 * 8, ret)
plt.axhline(y=0.7813)
